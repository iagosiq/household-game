...
export default function UserSelection() {
    ...
    const fetchSubUsers = async () => {
      if (user) {
        try {
          ...
          setSubUsers(subList);
          // Se nenhum subusuário foi selecionado, automaticamente seleciona o primeiro
  ...
  const handleConfirm = () => {
    if (selectedSubUser) {
      // Atualiza o localStorage e navega
      localStorage.setItem("activeSubUser", JSON.stringify({ name: selectedSubUser }));
      console.log("Subusuário confirmado:", { name: selectedSubUser });
      navigate("/dashboard");
    }
  };
